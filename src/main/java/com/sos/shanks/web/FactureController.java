...
public class FactureController extends Controller implements Serializable {
    ...
    public String addLivraisonToFacture(Livraison livraison){
        ...
        System.out.println(current.getLivraisonList().get(0).getNumeroLivraison());
        return showEdit(current);
    }
    
    public String doEdit(){
        System.out.println(current.getLivraisonList().get(0).getFournisseur().getRaisonSocial());
        factureService.edit(current);
        return showList();
    }
    // ======================================
    // = Getters & setters =
